...
提供两种使用方式:
1. 交互式菜单(show_menu): 无参数启动时进入
2. 命令行分发(dispatch): 带参数启动时直接执行
"""
...
from config.paths import BEST_MODEL_PATH, RAW_TEST_PATH, RAW_TRAIN_PATH, RAW_VAL_PATH
...
def menu_data_generate() -> None:
    ...
    train_size = int(_input_with_default("  训练集样本数", str(DataParams.TRAIN_SIZE)))
    val_size = int(_input_with_default("  验证集样本数", str(DataParams.VAL_SIZE)))
    test_size = int(_input_with_default("  测试集样本数", str(DataParams.TEST_SIZE)))

    _do_data_generate(train_size, val_size, test_size)
...
def menu_train() -> None:
    ...
    attention_type = _input_with_default(
        "  注意力类型 (bahdanau/luong/nadaraya_watson/none)",
        ModelParams.ATTENTION_TYPE,
    )
    ...
    bidirectional = (
        _input_with_default("  Encoder 双向? (y/N)", "n").strip().lower() == "y"
    )
...
def menu_predict() -> None:
    ...
    if mode == "1":
        date = input("  输入日期 (如 2024-01-15): ").strip()
        if not date:
            print("错误: 日期不能为空")
            return
        _do_predict(checkpoint_path, date=date)
...
def show_menu() -> None:
    ...
    while True:
        print()
        print("=" * 50)
        print("  DateConversion - Seq2Seq 日期格式转换")
        print("=" * 50)
        print("  1. 生成数据")
        print("  2. 训练模型")
        print("  3. 评估模型")
...
def dispatch(args: argparse.Namespace) -> None:
    ...
    与交互菜单共享核心函数。
    """
    if args.command == "data" and args.data_command == "generate":
        _do_data_generate(
